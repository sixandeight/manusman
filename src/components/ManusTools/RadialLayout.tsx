  ...
  return (
    ...
    >
      <div
        ...
        style={{
          background: "linear-gradient(145deg, rgba(30, 70, 140, 0.60), rgba(20, 50, 110, 0.48))",
          ...
          borderLeft: `3px solid ${
            ...
            card.phase === "thinking" ? "#fbbf24" :
            card.phase === "pending" ? "rgba(255,255,255,0.3)" :
            color
          }`,
          border: "1px solid rgba(147, 197, 253, 0.25)",
          borderLeftWidth: "3px",
          borderLeftColor: card.phase === "complete" ? "#4ade80" :
            card.phase === "thinking" ? "#fbbf24" :
            card.phase === "pending" ? "rgba(255,255,255,0.3)" :
            color,
          boxShadow: card.phase === "thinking"
            ? `0 4px 24px rgba(0,0,0,0.25), 0 0 12px rgba(251, 191, 36, 0.2), inset 0 1px 0 rgba(255,255,255,0.1)`
            : card.phase === "complete"
            ? `0 4px 24px rgba(0,0,0,0.25), 0 0 12px rgba(74, 222, 128, 0.2), inset 0 1px 0 rgba(255,255,255,0.1)`
      ...
      >
        <div className="flex items-center justify-between px-4 h-9 cursor-grab">
          <div className="flex items-center gap-2">
            <span className="text-[11px] font-bold uppercase tracking-widest" style={{ color }}>{label}</span>
            {card.isAuto && (
              <span className="text-[9px] font-medium uppercase px-1 py-0.5 rounded bg-white/10 text-white/60">auto</span>
            )}
          </div>
          <button onClick={() => onDismiss(card.id)} className="text-white/50 hover:text-white text-base font-bold leading-none w-5 h-5 flex items-center justify-center">×</button>
        </div>
        ...
        {card.phase === "input" && (
          <form onSubmit={submit} className="px-4 pb-3">
            <input ref={inputRef} value={card.query}
              ...
              className="w-full px-3 py-2 text-sm text-white bg-white/10 rounded-md border border-white/15 focus:outline-none focus:border-blue-400/50 placeholder-white/30"
              />
          </form>
        )}
        ...
        {(card.phase === "pending" || card.phase === "thinking") && (
          <div className="px-4 pb-3">
            {card.query && <div className="text-sm text-white/70 mb-2">{card.query}</div>}
            <div className="flex items-center gap-3">
              ...
              <span className="text-[10px] text-white/35">{phaseLabel}</span>
            </div>
          </div>
        )}
