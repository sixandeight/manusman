...
const StatPreset: React.FC<{ d: any; color: string }> = ({ d, color: _color }) => {
  ...
  return (
    <div className="space-y-4">
            ...
            return trend.map((v: number, i: number) => {
              ...
              return (
                <div key={i} className="flex-1 flex flex-col items-center justify-end" style={{ height: "100%" }}>
                  <div className="text-[12px] text-white/20 font-mono mb-1">{v}</div>
      ...
      {d.context && <div className="text-sm text-white/30">{d.context}</div>}
      {d.source && <div className="text-[13px] text-white/15">{d.source}</div>}
    </div>
  )
}
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-3">
        {(d.metrics || []).slice(0, 6).map((m: any, i: number) => {
          ...
          return (
            <div key={i}>
              <div className="text-[14px] text-white/40 mb-1.5 font-medium">{m.label}</div>
              <div className="grid grid-cols-2 gap-3">
                <div className="flex items-center gap-2">
                  <div className="flex-1 h-5 rounded bg-white/5 overflow-hidden">
                    <div className="h-full rounded" style={{ width: `${m.us_score * 10}%`, background: usWins ? "#4ade80" : "#4ade8044" }} />
                  </div>
                  <span className="text-sm text-white/50 font-mono w-6 text-right">{m.us_score}</span>
                </div>
                <div className="flex items-center gap-2">
                  <div className="flex-1 h-5 rounded bg-white/5 overflow-hidden">
                    <div className="h-full rounded" style={{ width: `${m.them_score * 10}%`, background: !usWins ? "#f87171" : "#f8717144" }} />
                  </div>
                  <span className="text-sm text-white/50 font-mono w-6 text-right">{m.them_score}</span>
                </div>
              </div>
            </div>
          )
      ...
      {d.verdict && <div className="text-base text-white/50 pt-4 border-t border-white/7">{d.verdict}</div>}
    </div>
  )
}
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-4">
        ...
        <div>
          <div className="text-xl font-bold text-white/90">{d.name}</div>
          <div className="text-base text-white/30">{d.role}{d.company ? ` · ${d.company}` : ""}</div>
        </div>
      </div>
      {d.details && d.details.length > 0 && (
        <div className="space-y-1.5 pl-1">
          {(d.details as string[]).slice(0, 5).map((detail: string, i: number) => (
            <div key={i} className="text-base text-white/50 flex items-start gap-2">
      ...
      {stageIdx >= 0 && (
        <div className="flex items-center gap-1.5">
          {stages.map((_, i) => (
            <div key={i} className="flex-1 h-2.5 rounded-full" style={{ background: i <= stageIdx ? "#4ade80" : "rgba(255,255,255,0.05)" }} />
          ))}
        </div>
      )}
      {d.summary && <div className="text-base text-white/40 italic">{d.summary}</div>}
    </div>
  )
}
...
const VerdictPreset: React.FC<{ d: any }> = ({ d }) => {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="text-base text-white/40 italic">"{d.claim}"</div>
      <div className="text-base text-white/50 leading-relaxed">{d.evidence}</div>
      {d.source && <div className="text-[13px] text-white/15">{d.source}</div>}
    </div>
  )
}
      ...
      {d.context && (
        <div className="space-y-2">
          {(d.context as any[]).slice(0, 5).map((c: any, i: number) => (
            <div key={i} className="flex items-start gap-2.5 text-base">
              <div className="w-1.5 h-full min-h-[18px] rounded-full shrink-0 mt-0.5" style={{ background: priorityColors[c.priority] || "#facc15" }} />
          ...
          <div className="space-y-2.5">
            {(d.items as any[]).slice(0, 5).map((item: any, i: number) => (
              <div key={i} className="flex items-center gap-3 text-base">
                <span className="w-5 h-5 rounded border-2 border-white/10 shrink-0" />
...
const PipelinePreset: React.FC<{ d: any }> = ({ d }) => {
  ...
  return (
    <div className="space-y-4">
        ...
        <div>
          <div className="flex gap-1.5">
            {stages.map((_: string, i: number) => (
              <div key={i} className="flex-1 h-5 rounded" style={{
          ...
          <div className="flex justify-between text-[13px] text-white/20 mt-2">
      ...
      {d.next_action && (
        <div className="p-4 rounded-lg bg-white/[0.03] border border-white/7">
          <div className="text-base font-semibold text-white/90">{d.next_action}</div>
          {d.next_action_due && <div className="text-sm text-white/30 mt-1">Due: {d.next_action_due}</div>}
        </div>
      )}
      <div className="flex items-center gap-4 text-sm">
        ...
        <span className="flex items-center gap-2">
          <span className="w-3.5 h-3.5 rounded-full" style={{ background: riskColor }} />
...
const ChartPreset: React.FC<{ d: any; color: string }> = ({ d, color }) => {
  ...
  if (!ds) return <div className="text-base text-white/40">No data</div>
          ...
          <div className="space-y-2">
            {ds.values.map((v: number, i: number) => (
              <div key={i} className="flex items-center gap-2.5 text-base">
                <span className="w-3.5 h-3.5 rounded-sm" style={{ background: colors[i] }} />
    ...
    <div className="space-y-3">
      ...
      <div className="flex items-end gap-2" style={{ height: 130 }}>
        {ds.values.map((v: number, i: number) => {
          ...
          return (
            <div key={i} className="flex-1 flex flex-col items-center justify-end" style={{ height: "100%" }}>
              <span className="text-[13px] text-white/40 font-mono mb-1">{v}</span>
      ...
      {d.summary && <div className="text-sm text-white/40 pt-1">{d.summary}</div>}
    </div>
  )
}
...
const SlidesPreset: React.FC<{ d: any; color: string }> = ({ d, color }) => {
  ...
  if (slides.length === 0) return <div className="text-base text-white/40">No slides</div>
  ...
  return (
    ...
    >
      {d.title && <div className="text-base font-medium text-white/40 mb-4">{d.title}</div>}

      <div className="p-4 rounded-lg bg-white/[0.03] border border-white/7 min-h-[120px]">
        <div className="text-sm font-bold uppercase tracking-wider mb-3" style={{ color }}>
          {slide.heading}
      ...
      {slides.length > 1 && (
        <div className="flex items-center justify-center gap-2.5 mt-4">
          <button
            ...
            className="text-white/15 hover:text-white/40 text-base px-1"
...
const FallbackPreset: React.FC<{ d: any }> = ({ d }) => (
  <div className="space-y-3">
    {d.title && <div className="text-lg font-medium text-white/90">{d.title}</div>}
    {d.fields?.map((f: any, i: number) => (
      <div key={i} className="flex gap-3 text-base">
        <span className="text-white/20 min-w-[100px]">{f.label}:</span>
        <span className="text-white/50">{f.value}</span>
      </div>
    ))}
    {d.summary && <div className="text-base text-white/40">{d.summary}</div>}
  </div>
)
