...
const ModelSelector: React.FC<ModelSelectorProps> = ({ onModelChange, onChatOpen }) => {
  ...
  return (
    <div className="p-4 bg-white/20 backdrop-blur-md rounded-lg border border-white/30 space-y-4">
      ...
      {/* Current Status */}
      {currentConfig && (
        <div className="text-sm text-gray-600 bg-white/40 p-2 rounded">
      ...
      {selectedProvider === 'gemini' ? (
        <div className="space-y-2">
          <label className="text-sm font-medium text-gray-700">Gemini API Key (optional if already set)</label>
          <input
            type="password"
            placeholder="Enter API key to update..."
            value={geminiApiKey}
            onChange={(e) => setGeminiApiKey(e.target.value)}
            className="w-full px-3 py-2 text-sm bg-white/40 border border-white/60 rounded focus:outline-none focus:ring-2 focus:ring-blue-400/60"
          />
        </div>
      ) : (
        <div className="space-y-2">
          <div>
            ...
            <input
              ...
              className="w-full px-3 py-2 text-sm bg-white/40 border border-white/60 rounded focus:outline-none focus:ring-2 focus:ring-green-400/60"
            />
          </div>
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className="px-2 py-1 text-sm bg-white/60 hover:bg-white/80 rounded transition-all"
            ...
            {availableOllamaModels.length > 0 ? (
              ...
              <div className="text-sm text-gray-600 bg-yellow-100/60 p-2 rounded">
      ...
      <div className="flex gap-2 pt-2">
        <button
          ...
          className="flex-1 px-3 py-2 bg-blue-500 hover:bg-blue-600 disabled:bg-gray-400 text-white text-sm rounded transition-all shadow-md"
        >
        ...
        <button
          ...
          disabled={connectionStatus === 'testing'}
          className="px-3 py-2 bg-gray-500 hover:bg-gray-600 disabled:bg-gray-400 text-white text-sm rounded transition-all shadow-md"
        >
          Test
        </button>
      </div>
