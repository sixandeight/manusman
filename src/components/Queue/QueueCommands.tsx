...
const QueueCommands: React.FC<QueueCommandsProps> = ({
  ...
  const btnDefault = `${btnClass} bg-white/10 hover:bg-white/20 text-white font-semibold`
  const btnAccent = `${btnClass} bg-blue-500/30 hover:bg-blue-500/50 text-white font-semibold border border-blue-400/30`
  ...
  return (
    <div className="w-fit">
      ...
      <div className="flex items-center gap-2 py-2 px-3 rounded-lg" style={{ background: "rgba(20, 20, 30, 0.85)", border: "1px solid rgba(255,255,255,0.15)" }}>
        ...
        <button className={btnAccent} onClick={() => window.electronAPI.invoke("trigger-manus-tool", "prep")}>
          3 Prep
        </button>
        <button className={btnAccent} onClick={() => window.electronAPI.invoke("trigger-manus-tool", "live_fact_check")}>
          4 Fact
        </button>
        ...
        <button
          className="px-2 py-1.5 rounded-md text-red-300 hover:text-red-200 hover:bg-red-500/15 transition-colors"
          title="Quit"
      ...
      {showKeybinds && (
        ...
        >
          <div className="text-sm font-semibold text-white mb-2">Keybinds</div>
          <div className="space-y-1">
            {KEYBINDS.map((kb, i) => (
              kb.keys === "─" ? (
                <div key={i} className="text-xs text-white/50 pt-1">{kb.label}</div>
              ) : (
                <div key={i} className="flex justify-between items-center text-sm">
                  <span className="text-white/90">{kb.label}</span>
                  <span className="text-xs font-mono bg-white/10 px-2 py-0.5 rounded text-white/75">{kb.keys}</span>
                </div>
              )
            ))}
          </div>
